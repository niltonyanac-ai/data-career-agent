...
st.markdown("""
    ...
    .match-badge { background-color: #2563eb; color: white; padding: 4px 12px; border-radius: 20px; font-weight: bold; }
    </style>
...
with tab1:
    ...
    with col1:
        ...
        st.altair_chart(alt.Chart(hards['hard_skills'].value_counts().reset_index()).mark_bar().encode(x='count:Q', y=alt.Y('hard_skills:N', sort='-x')), use_container_width=True)
    with col2:
        ...
        st.altair_chart(alt.Chart(softs['soft_skills'].value_counts().reset_index()).mark_bar().encode(x='count:Q', y=alt.Y('soft_skills:N', sort='-x')), use_container_width=True)
...
with tab2:
    ...
    if archivo:
        if "GEMINI_API_KEY" not in st.secrets:
            st.error("Configura la clave GEMINI_API_KEY en los Secrets de Streamlit.")
        else:
            try:
                reader = PdfReader(archivo)
                texto = "".join([p.extract_text() for p in reader.pages if p.extract_text()])
                if not texto: raise ValueError("El archivo PDF no contiene texto legible.")
                ...
                with st.spinner("Analizando tu perfil con IA..."):
                    resp = cliente_ai.models.generate_content(
                        ...
                        contents=f"CV: {texto[:2000]} \n Vacantes disponibles: {df_f.head(10).to_json()}", 
                    ...
                    for res in datos['evaluaciones']:
                        ...
                        if not match_info.empty:
                            ...
                            <div class='match-card'>
                                ...
                                <a href='{info['link']}' target='_blank'>🔗 Ver oferta en LinkedIn</a>
                            </div>
                            """, unsafe_allow_html=True)
